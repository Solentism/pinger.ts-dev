...
export const ping = async function (url:string, interval:number=30000, logging:boolean=true, flipDM:boolean=false, noURLFix:boolean=false): Promise<void|ReturnType<typeof setInterval>> {
    ...
    if (!url) return console.log(`[🏓 pinger.ts-dev] You must specify a URL.`);
    ...
    if (URLValidity(url) !== true || encodeURIComponent(url).includes("%3C" || "%3E" || "%20")) {
        return console.log(`[🏓 pinger.ts-dev] You must specify a valid URL.`);
    }

    if (logging == true){
        console.log(`[🏓 pinger.ts-dev] Currently logging pings for ${url} with the interval ${interval}.`);
    }
    ...
    return setInterval(async () => {
        ...
        await axios.get(url, {
            headers: {
                'User-Agent': 'npmjs.org/pinger.ts-dev'
            },
        })
        .catch(e => console.log(`[🏓 pinger.ts-dev] [${dm(d.getMonth() + 1, d.getDate())} ${ps(d.getHours())}:${ps(d.getMinutes())}:${ps(d.getSeconds())}] Failed to ping ${url}. Error: `, e));

        if (logging === true) {
            console.log(`[🏓 pinger.ts-dev] [${dm(d.getMonth() + 1, d.getDate())} ${ps(d.getHours())}:${ps(d.getMinutes())}:${ps(d.getSeconds())}] Successfully pinged ${url} -> Ping #${count}`);
        };
...
export const webserver = async function (port:number|string, content:any, flipDM:boolean=false):Promise<void> {
    ...
    app.get('*', (req, res) => {
        if (!content) { 
            res.json('[🏓 pinger.ts-dev] Actively listening for requests. If logging is enabled, check your terminal.')
        } else {
            res.send(content)
        }
    });
    ...
    const listener = app.listen(port, () => {
        ...
        console.log(`[🏓 pinger.ts-dev] [${dm(d.getMonth() + 1, d.getDate())} ${ps(d.getHours())}:${ps(d.getMinutes())}:${ps(d.getSeconds())}] Webserver listening on port ${port}`);
    });
}
...
export const dping = async function (webhooktoken:string, webhookid:string|number, url:string, interval:number, flipDM:boolean=false, noURLFix:boolean=false):Promise<void|ReturnType<typeof setInterval>> {
    ...
    if(!webhooktoken) return console.log(`[🏓 pinger.ts-dev] You must specify a webhook token. If you don't want Discord logging, use the .ping method.`);
    if(!webhookid) return console.log(`[🏓 pinger.ts-dev] You must specify a webhook id. If you don't want Discord logging, use the .ping method.`);

    if (!interval) interval = 30000;
    if (!url) return console.log(`[🏓 pinger.ts-dev] You must specify a URL.`);

    if (interval < 10000) return console.log(`[🏓 pinger.ts-dev] Due to Discord's API rate limiting, you cannot send more than one request in under 10 seconds. Try using an interval of 10000 or more.`);
    ...
    console.log(`[🏓 pinger.ts-dev] Currently logging pings for ${url} with the interval ${interval}. Please check your Discord channel connected to the webhook.`);
    ...
    return setInterval(async () => {
        ...
        webhookClient.send({
            content: `[🏓 pinger.ts-dev] [${dm(d.getMonth() + 1, d.getDate())} ${ps(d.getHours())}:${ps(d.getMinutes())}:${ps(d.getSeconds())}] Successfully pinged <${url}> -> Ping #${count}`,
            username: 'pinger.ts-dev Logging'
        });
    ...
    public add(url:string, interval:number, logging:boolean=true, flipDM:boolean=false, noURLFix:boolean=false) {
        ...
        let ping = {
            ...
            ping: async function(url:string, interval:number, logging:boolean=true, flipDM:boolean=false, noURLFix:boolean=false):Promise<void|ReturnType<typeof setInterval>> {
                let count = 1;
                const pf = `[🏓 pinger.ts-dev] [${this.name}] [${this.id}]`;
        ...
        console.log(`[🏓 pinger.ts-dev] [${ping.name}] Added pinger for ${ping.config.url} into group.`);
